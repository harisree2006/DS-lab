...
int main() {
    ...
    int visited[100] = {0}; // To track if process is already in queue
    ...
    // Initial push: add processes that arrive at time 0
    for(int i = 0; i < n; i++) {
    ...
    while (completed != n) {
        ...
        if (p[idx].remaining_bt > 0) {
            // If not finished, put back to end of queue
            queue[rear++] = idx;
        } else {
            // Process Finished
        ...
        // If queue is empty but not all processes completed (CPU Idle)
